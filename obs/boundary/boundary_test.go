...
// Package boundary_test guards the module boundary that obs exists to create.
//
// lib-service-discovery must not name a
// github.com/LerianStudio/lib-observability type anywhere on its public API.
// Go types have nominal identity, so a single exported field or parameter of a
// lib-observability type re-pins every consumer of lib-service-discovery to
// whichever lib-observability major this library chose — and forces a major
// here every time lib-observability takes one. This test walks every non-test
// .go file in the module with go/ast and fails if any exported symbol mentions
// such a type.
//
...
// root package here — which still imports log/ for log.String and friends at
// its own internal call sites, and runtime/ for SafeGo — never has to NAME one
// of its types on anything exported. No package of this module has an excuse.
// Adding an entry re-opens the coupling obs exists to close.
var allowedPackages = map[string]bool{}

// skippedDirs are directories that are not part of the library's API surface.
var skippedDirs = map[string]bool{
	// services/ is the docker-compose demo binary (package main), a consumer of
	// the library rather than part of it.
	"services": true,
}
...
func TestPublicAPI_DoesNotMentionLibObservability(t *testing.T) {
	...
	// The module root: obs/boundary -> obs -> .
	...
	err = filepath.WalkDir(root, func(path string, entry fs.DirEntry, err error) error {
		...
		if entry.IsDir() {
			if strings.HasPrefix(entry.Name(), ".") && slash != "." {
				return fs.SkipDir
			}

			if skippedDirs[slash] {
				return fs.SkipDir
			}
		...
		if strings.Contains(slash, "/internal/") || strings.HasPrefix(slash, "internal/") {
			return nil
		}

		found, scanErr := scanFile(path, slash)
		if scanErr != nil {
			return scanErr
		}
	...
	if !assert.Empty(t, violations, "lib-observability types must not appear on the public API of lib-service-discovery") {
